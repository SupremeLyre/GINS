...
#define fdKG9(dkii, dkij)                                                                                              \
    (dkii) * PPM, (dkij)*SEC, (dkij)*SEC, (dkij)*SEC, (dkii)*PPM, (dkij)*SEC, (dkij)*SEC, (dkij)*SEC, (dkii)*PPM
...
#define CC180C360(yaw)                                                                                                 \
    ((yaw) > 0.0 ? (_2PI - (yaw)) : -(yaw)) // counter-clockwise +-180deg -> clockwise 0~360deg for yaw
#define C360CC180(yaw)                                                                                                 \
    ((yaw) >= PI ? (_2PI - (yaw)) : -(yaw)) // clockwise 0~360deg -> counter-clockwise +-180deg for yaw
...
class CGLV
{
  public:
    ...
    CGLV(double Re = 6378137.0, double f = (1.0 / 298.257223563), double wie0 = 7.2921151467e-5,
...
class CVect3
{
  public:
    ...
    friend CVect3 dv2att(const CVect3 &va1, const CVect3 &va2, const CVect3 &vb1,
...
class CMat3
{
  public:
    ...
    CMat3(double xx, double xy, double xz, double yx, double yy, double yz, double zx, double zy, double zz);
...
{
  public:
    double max0, min0, maxpre0, minpre0, maxCur, minCur, maxpreCur, minpreCur, maxRes, minRes, maxpreRes, minpreRes;
...
class CSINS // sizeof(CSINS)~=3k bytes
{
  public:
    ...
    CVect3 wib, fb, fn, an, anbar, web, webbar, wnb, att, vn, mvn, mvni, mvnmax, mvnmin, vb, pos, eb, db, Ka2, tauGyro,
        tauAcc, _betaGyro, _betaAcc;
    ...
    void Init(const CQuat &qnb0 = qI, const CVect3 &vn0 = O31, const CVect3 &pos0 = O31,
    ...
    void Extrap(const CVect3 &wm = O31, const CVect3 &vm = O31,
                double ts = 0.0); // SINS fast extrapolation using 1 Gyro&Acc sample
...
class CIIRV3
{
  public:
    ...
    CIIRV3(void);
    CIIRV3(double *b0, double *a0, int n0, double *b1 = (double *)NULL, double *a1 = (double *)NULL, int n1 = 0,
...
class CKalman //抽象类
{
  public:
    ...
    CVect Xk, Zk, Qt, Rt, rts, RtTau, measstop, measlost, Xmax, Pmax, Pmin, Pset, Zfd, Zfd0, Rmax, Rmin, Rbeta, Rb,
        Rstop,                                      // measurement noise R adaptive
...
class CSINSTDKF : public CKalman
{
  public:
    ...
    int iter, ifn, adptOKi, measRes, tdStep, maxStep, curOutStep,
        maxOutStep; // tdStep:当前分片滤波一次次数  maxStep:分片滤波一次最大次数
    CMat Fk, Pk1;
    ...
    void TDReset(void);
    int TDUpdate(const CVect3 *pwm, const CVect3 *pvm, int nSamples, double ts,
                 int nStep = 1); // Time-Distributed Update
...
class CVAutoPOS
    : public CSINSGNSSOD // high-accuracy Vehicluar Autonomous Positioning & Orientation System, i.e. SINS/OD
{
  public:
    CVAutoPOS(void);
    CVAutoPOS(double ts);
...
typedef struct
{
    float hd, t, gx, gy, gz, ax, ay, az, magx, magy, magz, bar, pch, rll, yaw, ve, vn, vu, lon0, lon1, lat0, lat1, hgt,
        gpsve, gpsvn, gpsvu, gpslon0, gpslon1, gpslat0, gpslat1, gpshgt, gpsstat, gpsdly, tmp, sum;
} PSINSBoard;
...
#define UARTBUFLEN (UARTFRMLEN * 100)
    ...
    int csflag, cs0, cs1,
        css; // 0: no checksum, 1: unsigned char sum, 2: crc8; popbuf[css]==popbuf[cs0]+...+popbuf[cs1]
