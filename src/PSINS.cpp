...
const CVect3 O31(0.0), One31(1.0),
    Ipos(1.0 / RE, 1.0 / RE, 1.0), posNWPU = LLH(34.246048, 108.909664, 380); // NWPU-Lab-pos
...
{
    double a3 = a / 3, p = b - a * a3,
           q = 2.0 / 27 * a * a * a - a3 * b + c; // p=(3*b-a*a)/3, q=(2*a*a*a-9*a*b+27*c)/27;
    CComplex x1, x2, x3;
    if (p == 0.0 && q >= 0.0)
    {
    ...
    else
    {
        ...
        CComplex rp60(0.5, sqrt3 / 2.0), rn60(0.5, -sqrt3 / 2.0), p3D = p / 3 / Delta1;
        x1 = -(p3D - Delta1), x2 = rp60 * p3D - rn60 * Delta1, x3 = rn60 * p3D - rp60 * Delta1;
    }
...
{
    return CMat3(v1.i * v2.i, v1.i * v2.j, v1.i * v2.k, v1.j * v2.i, v1.j * v2.j, v1.j * v2.k, v1.k * v2.i, v1.k * v2.j,
                 v1.k * v2.k);
}
...
{
    double i = v.i > 0 ? v.i : -v.i, j = v.j > 0 ? v.j : -v.j, k = v.k > 0 ? v.k : -v.k;
    if (i > j)
        return i > k ? i : k;
    else
        return j > k ? j : k;
...
{
    return CMat3(0, -v.k, v.j, v.k, 0.0, -v.i, -v.j, v.i, 0);
}

CMat3 pos2Cen(const CVect3 &pos)
{
    double si = sin(pos.i), ci = cos(pos.i), sj = sin(pos.j), cj = cos(pos.j);
    return CMat3(-sj, -si * cj, ci * cj, cj, -si * sj, ci * sj, 0, ci, si); // Cen
}
...
{
    double s = normXY(xyz), theta = atan2(xyz.k * glv.Re, s * glv.Rp), s3 = sin(theta), c3 = cos(theta);
    s3 = s3 * s3 * s3, c3 = c3 * c3 * c3;
    if (s < (6378137.0 * 1.0 * DEG))
        return O31;
    double L = atan2(xyz.j, xyz.i), B = atan2(xyz.k + glv.ep2 * glv.Rp * s3, s - glv.e2 * glv.Re * c3), sB = sin(B),
           cB = cos(B), N = glv.Re / sqrt(1 - glv.e2 * sB * sB);
    return CVect3(B, L, s / cB - N);
}

CVect3 blh2xyz(const CVect3 &blh)
{
    double sB = sin(blh.i), cB = cos(blh.i), sL = sin(blh.j), cL = cos(blh.j), N = glv.Re / sqrt(1 - glv.e2 * sB * sB);
...
CMat3::CMat3(double xx, double xy, double xz, double yx, double yy, double yz, double zx, double zy, double zz)
{
...
{
    double si = sin(att.i), ci = cos(att.i), sj = sin(att.j), cj = cos(att.j), sk = sin(att.k), ck = cos(att.k);
    CMat3 Cnb;
...
CMat3 q2mat(const CQuat &qnb)
{
    double q11 = qnb.q0 * qnb.q0, q12 = qnb.q0 * qnb.q1, q13 = qnb.q0 * qnb.q2, q14 = qnb.q0 * qnb.q3,
           ...
           q34 = qnb.q2 * qnb.q3, q44 = qnb.q3 * qnb.q3;
    CMat3 Cnb;
    Cnb.e00 = q11 + q22 - q33 - q44, Cnb.e01 = 2 * (q23 - q14), Cnb.e02 = 2 * (q24 + q13), Cnb.e10 = 2 * (q23 + q14),
    Cnb.e11 = q11 - q22 + q33 - q44, Cnb.e12 = 2 * (q34 - q12), Cnb.e20 = 2 * (q24 - q13), Cnb.e21 = 2 * (q34 + q12),
...
double trMMT(const CMat3 &m1, const CMat3 &m2 = I33)
{
    const CMat3 *pm2;
    pm2 = (&m2 == &I33) ? &m1 : &m2;
    return m1.e00 * pm2->e00 + m1.e01 * pm2->e01 + m1.e02 * pm2->e02 + m1.e10 * pm2->e10 + m1.e11 * pm2->e11 +
           m1.e12 * pm2->e12 + m1.e20 * pm2->e20 + m1.e21 * pm2->e21 + m1.e22 * pm2->e22;
}
...
{
    double D = -8 * b, E = -8 * c, F = 16 * b * b - 64 * d, E2 = E * E, A = D * D - 3 * F, B = D * F - 9 * E2,
           C = F * F - 3 * D * E2; // Delta=B*B-4*A*C;
...
void DVMDVafa(const CVect &V, CMat &M, double afa = 1.0)
{
    ...
    {
        for (double *prow = &M.dd[i * M.clm], *prowEnd = prow + M.clm, *pclm = &M.dd[i]; prow < prowEnd;
             prow++, pclm += M.row)
        {
            *prow *= vi;
            *pclm *= viafa;
...
CIIRV3::CIIRV3(double *b0, double *a0, int n0, double *b1, double *a1, int n1, double *b2, double *a2, int n2)
{
    iir0 = CIIR(b0, a0, n0);
    if (n1 == 0)
...
void CKalman::XPConstrain(void)
{
         ...
         pmin < pminEnd; px++, pxmax++, p += nq1, pmin++, pmax++, pset++)
    {
        ...
        else if (*p > *pmax)
        {
            ...
            for (double *prow = &Pk.dd[i * Pk.clm], *prowEnd = prow + nq, *pclm = &Pk.dd[i]; prow < prowEnd;
                 prow++, pclm += nq)
            {
                *prow *= sqf;
                *pclm = *prow;
...
{
    for (double *p = Pk.dd, *pmax = Pmax.dd, *pmin = Pmin.dd, *pminEnd = &Pmin.dd[nq]; pmin < pminEnd;
         p += nq + 1, pmax++, pmin++)
    {
        if (*p > *pmax)
            *pmax = *p * 10.0;
...
{
    double *pTau = FBTau.dd, *pTotal = FBTotal.dd, *pMax = FBMax.dd, *pOne = FBOne.dd, *pOne1 = FBOne1.dd,
           *pXk = FBXk.dd, *p = Xk.dd;
...
void fusion(double *x1, double *p1, const double *x2, const double *p2, int n = 9, double *xf = NULL, double *pf = NULL)
{
    ...
    CVect3 att1;
    if (n < 100)
    { // n<100 for att(1:3), n>100 for no_att(1:3)
        x10 = x1, xf0 = xf;
        att1 = *(CVect3 *)x1;
...
void CSINSGNSS::Init(const CSINS &sins0, int grade)
{
    ...
    sins.lever(-lvGNSS);
    sins.pos = sins.posL; // sins0.pos is GNSS pos
    avpi.Init(sins, kfts, 1);
    if (grade == 0)
    { // inertial grade
        Pmax.Set2(fDEG3(1.0), fXXX(100.0), fdPOS(1.0e6), fDPH3(0.5), fMG3(1.0), fXXX(100.0), 0.5, fdKG9(1000.0, 900.0),
                  fdKA6(100.0, 100.0));
        Pmin.Set2(fPHI(0.1, 1.0), fXXX(0.001), fdPOS(.01), fDPH3(0.001), fUG3(10.0), fXXX(0.001), 0.0001,
                  fdKG9(1.0, 1.0), fdKA6(1.0, 1.0));
        Pk.SetDiag2(fPHI(60, 600), fXXX(1.0), fdPOS(100.0), fDPH3(0.1), fMG3(1.0), fXXX(1.0), 0.01, fdKG9(100.0, 90.0),
                    fdKA6(10.0, 10.0));
        Qt.Set2(fDPSH3(0.001), fUGPSHZ3(1.0), fOOO, fOO6, fOOO, 0.0, fOO9, fOO6);
        // MarkovGyro(I31*1000.0, I31*0.01*DPH);  MarkovAcc(I31*1000.0, I31*1.0*UG);
        Xmax.Set(fINF9, fDPH3(0.5), fMG3(1.0), fXXX(10.0), 0.5, fdKG9(1000.0, 900.0), fdKA6(1000.0, 900.0));
    ...
    { // MEMS grade
        Pmax.Set2(fDEG3(50.0), fXXX(100.0), fdPOS(1.0e6), fDPH3(5000.0), fMG3(30.0), fXXX(100.0), 0.5,
                  fdKG9(1000.0, 900.0), fdKA6(100.0, 100.0));
        Pmin.Set2(fPHI(1, 1), fXXX(0.0001), fdPOS(.001), fDPH3(0.001), fUG3(1.0), fXXX(0.001), 0.0001, fdKG9(1.0, 1.0),
                  fdKA6(1.0, 1.0));
        Pk.SetDiag2(fPHI(600, 600), fXXX(1.0), fdPOS(100.0), fDPH3(1000.0), fMG3(10.0), fXXX(1.0), 0.01,
                    fdKG9(1000.0, 90.0), fdKA6(10.0, 10.0));
        Qt.Set2(fDPSH3(1.1), fUGPSHZ3(10.0), fOOO, fOO6, fOOO, 0.0, fOO9, fOO6);
        Xmax.Set(fINF9, fDPH3(3600.0), fMG3(50.0), fXXX(10.0), 0.5, fdKG9(1000.0, 900.0), fdKA6(1000.0, 900.0));
        Rt.Set2(fXXZ(0.5, 1.0), fdLLH(10.0, 30.0));
        Rmax = Rt * 100;
        Rmin = Rt * 0.01;
        Rb = 0.6;
...
void CSINSGNSS::Feedback(int nnq, double fbts)
{
    ...
    {
        CMat3 IdKGz(1.0, 0.0, -FBXk.dd[19], 0.0, 1.0, -FBXk.dd[20], 0.0, 0.0,
                    1.0 - FBXk.dd[21]); // 正常的处理逻辑单位阵操作
    ...
    if (nnq >= 34)
    {
        CMat3 IdKA(1.0 - FBXk.dd[28], 0.0, 0.0, -FBXk.dd[29], 1.0 - FBXk.dd[31], 0.0, -FBXk.dd[30], -FBXk.dd[32],
                   1.0 - FBXk.dd[33]);
...
BOOL CPOS618::Load(char *fname, double t0, double t1)
{
    ...
    for (iter = 0, pIG = (ImuGnssData *)pmemImuGnss->get(0); iter < records; iter++, pIG++)
    { // find first pos0
...
void CSINSGNSSDR::Init(const CSINS &sins0, int grade)
{
    ...
    avpi.Init(sins, kfts);
    Pmax.Set2(fDEG3(10.0), fXXX(50.0), fdPOS(1.0e4), fDPH3(3600), fMG3(10.0), 1 * DEG, 0.01, 1 * DEG, fdPOS(1.0e4));
    Pmin.Set2(fPHI(10.0, 60.0), fXXX(0.01), fdPOS(0.1), fDPH3(0.1), fUG3(100), 0.01 * DEG, 0.001, 0.01 * DEG,
              fdPOS(0.1));
    Pk.SetDiag2(fDEG3(10.0), fXXX(1.0), fdPOS(100.0), fDPH3(1000), fMG3(1.0), 1 * DEG, 0.01, 1 * DEG, fdPOS(100.0));
...
void CSINSGNSSDR::SetFt(int nnq) // 系统模型需要重写
{
    ...
    CMat3 MvkD = norm(sins.vn) * CMat3(-sins.Cnb.e02, sins.Cnb.e01, sins.Cnb.e00, -sins.Cnb.e12, sins.Cnb.e11,
                                       sins.Cnb.e10, -sins.Cnb.e22, sins.Cnb.e21, sins.Cnb.e20);
...
void CSINSGNSSDR::SetMeasGNSS(const CVect3 &pgnss, const CVect3 &vgnss, double yawgnss)
{
    ...
    if (!IsZero(yawgnss) && IsZero(sins.att.i, 10 * DEG) && IsZero(sins.att.j, 10 * DEG) &&
        IsZero(sins.wnb.k, 10 * DPS))
    {
...
int CSINSGNSSOD::ODVelUpdate(double dS)
{
    ...
    dS *= Kod;
    distance += dS;
    CVect3 dSn = sins.Cnb * (CVect3(Cbo.e01 * dS, Cbo.e11 * dS, Cbo.e21 * dS) -
                             sins.imu.phim * lvOD); // dSn = Cnb*Cbo*dSb（转向也可以使用）
    odmeast += sins.nts;
    if (odmeast >= odmeasT)
    { // take measurement every (odmeasT)(s)
        odMphi = -askew(odmeast / 2.0 * Ifn + IVno);
        odMvn = odmeast * I33;
        odMkappa = CMat3(IMv.e02, -IMv.e01, -IMv.e00, IMv.e12, -IMv.e11, -IMv.e10, IMv.e22, -IMv.e21, -IMv.e20);
...
void CAutoDrive::Init(const CSINS &sins0, int grade)
{
    ...
    Rt.Set2(fXXZ(0.2, 0.6), fdLLH(10.0, 30.0), fXXZ(0.1, 0.1), fXXZ(0.1, 0.1), fXYZ(0.1, 10.0, 0.1), 1.0 * DEG,
            10.0 * DPH);
    Rmax = Rt * 100;
    Rmin = Rt * 0.01;
...
void CAutoDrive::ZUPTtest(void)
{
    if (sins.mmwb.flag == 1)
    {
        double dwb = sins.mmwb.maxRes - sins.mmwb.minRes, dfb = sins.mmfb.maxRes - sins.mmfb.minRes;
...
int CAutoDrive::Update(const CVect3 *pwm, const CVect3 *pvm, double dS, int nn, double ts, int nSteps)
{
    ...
    if (odVelOK)
    {
        Hk.SetMat3(6, 0, odMphi);
        Hk.SetMat3(6, 3, odMvn);
        Hk.SetMat3(6, 15, odMkappa); // 里程计作用
...
void CSGOClbt::Init(const CSINS &sins0, int grade)
{
    ...
    if (grade < 0)
        return;
    Pmax.Set2(fDEG3(10.0), fXXX(50.0), fdPOS(1.0e4), fDPH3(3600), fMG3(10.0), fXXX(1.1), fKPP(1, 0.01, 1), fXXX(10.0),
              0.1, 1 * DEG);
    Pmin.Set2(fPHI(0.1, 0.6), fXXX(0.001), fdPOS(0.01), fDPH3(0.1), fUG3(100), fXXZ(0.01, 0.01),
              fKPP(0.01, 0.001, 0.01), fXXX(0.001), 0.0001, 0.01 * DEG);
    Pk.SetDiag2(fDEG3(1.0), fXXX(1.0), fdPOS(100.0), fDPH3(10), fMG3(1.0), fXXZ(1.1, 1.1), fKPP(0.1, 0.01, 1.1),
                fXXX(1.0), 0.01, 1.1 * DEG);
...
int CSGOClbt::Update(const CVect3 *pwm, const CVect3 *pvm, double dS, int nn, double ts, int nSteps)
{
    ...
    if (odVelOK)
    {
        Hk.SetMat3(6, 0, odMphi);
        Hk.SetMat3(6, 3, odMvn);
        Hk.SetMat3(6, 18, odMkappa);
        Hk.SetMat3(6, 21, odMlever); // lvOD
        *(CVect3 *)&Zk.dd[6] = odZk; // SINS/OD-dvn
        SetMeasFlag(0700);
    }
    return res;
...
{
    CSINSGNSSOD::Init(sins0, grade);
    Pmin.Set2(fPHI(0.01, .1), fXXX(0.0001), fdPOS(0.001), fDPH3(0.001), fXYZU(10, 10, 50, UG), 0.0 * DEG, 0.000,
              0.0 * DEG);
    // States   0-14: phi,dvn,dpos,eb,db; 15-17: Kappa;
    Pk.SetDiag2(fPHI(3.0, 10.0), fXXX(0.5), fdPOS(1.0), fDPH3(0.05), fXYZU(50, 50, 1000, UG), 0.5 * DEG, 0.01,
                0.50 * DEG);
...
int CVAutoPOS::Update(const CVect3 *pwm, const CVect3 *pvm, double dS, int nn, double ts, int nSteps)
{
    ...
    if (odVelOK)
    {
        Hk.SetMat3(0, 0, odMphi);
        Hk.SetMat3(0, 3, odMvn);
...
void CSINS::Update(const CVect3 *pwm, const CVect3 *pvm, int nSamples, double ts)
{
    ...
    if (pos.k > hgtMax)
        ...
        pos.k = hgtMin;
    if (mvnT > EPS)
    { // calculate max/min/mean-vn within mvnT
        if (mvnk == 0)
        {
            mvnmax = mvnmin = vn;
            mvni = O31;
...
void CSINS::etm(void)
{
    ...
    {
        double tl = eth.tl, secl = 1.0 / eth.cl, secl2 = secl * secl, wN = eth.wnie.j, wU = eth.wnie.k, vE = vn.i,
               vN = vn.j;
...
void CQEAHRS::Update(const CVect3 &gyro, const CVect3 &acc, const CVect3 &mag, double ts)
{
    ...
    // Ft
    0, Ft.dd[1] = -wx / 2, Ft.dd[2] = -wy / 2, Ft.dd[3] = -wz / 2, Ft.dd[4] = q1 / 2, Ft.dd[5] = q2 / 2,
       Ft.dd[6] = q3 / 2;
    Ft.dd[7] = wx / 2, 0, Ft.dd[9] = wz / 2, Ft.dd[10] = -wy / 2, Ft.dd[11] = -q0 / 2, Ft.dd[12] = q3 / 2,
    Ft.dd[13] = -q2 / 2;
    Ft.dd[14] = wy / 2, Ft.dd[15] = -wz / 2, 0, Ft.dd[17] = wx / 2, Ft.dd[18] = -q3 / 2, Ft.dd[18] = -q0 / 2,
    Ft.dd[20] = q1 / 2;
    Ft.dd[21] = wz / 2, Ft.dd[22] = wy / 2, Ft.dd[23] = -wx / 2, 0, Ft.dd[25] = q2 / 2, Ft.dd[26] = -q1 / 2,
    Ft.dd[27] = -q0 / 2;
    // Hk
    ...
    /*	CVect3 magH = Cnb*mag;
        ...
        {
            double f2=2.0/CC;
            Hk.dd[14] = (q3*C11+q0*C01)*f2,  Hk.dd[15] = (-q2*C11-q1*C01)*f2,  Hk.dd[16] = (-q1*C11+q2*C01)*f2,
       Hk.dd[17] = (q0*C11-q3*C01)*f2; Zk.dd[2] = atan2(magH.i, magH.j);
        }
        else
        {
...
void CFileRdWt::Init(const char *fname0, int columns0)
{
    ...
    else if (columns > 0) // txt file read
    {
        ...
        {
            sstr[4 * i + 0] = '%', sstr[4 * i + 1] = 'l', sstr[4 * i + 2] = 'f', sstr[4 * i + 3] = ' ',
                         sstr[4 * i + 4] = '\0';
        }
    }
    else
...
int CFileRdWt::load(int lines, BOOL txtDelComma)
{
    ...
    else // txt file read
    {
        ...
        if (columns < 10)
            sscanf(line, sstr, &buff[0], &buff[1], &buff[2], &buff[3], &buff[4], &buff[5], &buff[6], &buff[7], &buff[8],
                   &buff[9]);
        else if (columns < 20)
            sscanf(line, sstr, &buff[0], &buff[1], &buff[2], &buff[3], &buff[4], &buff[5], &buff[6], &buff[7], &buff[8],
                   &buff[9], &buff[10], &buff[11], &buff[12], &buff[13], &buff[14], &buff[15], &buff[16], &buff[17],
                   &buff[18], &buff[19]);
        else if (columns < 40)
            sscanf(line, sstr, &buff[0], &buff[1], &buff[2], &buff[3], &buff[4], &buff[5], &buff[6], &buff[7], &buff[8],
                   &buff[9], &buff[10], &buff[11], &buff[12], &buff[13], &buff[14], &buff[15], &buff[16], &buff[17],
                   &buff[18], &buff[19], &buff[20], &buff[21], &buff[22], &buff[23], &buff[24], &buff[25], &buff[26],
                   &buff[27], &buff[28], &buff[29], &buff[30], &buff[31], &buff[32], &buff[33], &buff[34], &buff[35],
...
CFileRdWt &CFileRdWt::operator<<(const CMaxMinn &mm)
{
    for (int i = 0; i < mm.n; i++)
        *this << mm.mm[i].maxRes << mm.mm[i].minRes << mm.mm[i].maxpreRes << mm.mm[i].minpreRes
              << (double)mm.mm[i].flag;
    return *this;
}
...
void CAligntrkang::Init(const CSINS &sins0)
{
    ...
    Hk(3, 2) = 1.0; // phi-meas
    Pmin.Set2(0.1 * glv.min, 0.1 * glv.min, 10 * glv.min, 0.01, 0.01, 0.01, 1.0 / glv.Re, 1.0 / glv.Re, 0.1, 1 * DPH,
              1 * DPH, 1 * DPH, 10.0 * glv.ug, 10.0 * glv.ug, 10.0 * glv.ug);
    Pk.SetDiag2(10.0 * DEG, 10. * DEG, 10.0 * DEG, 10.0, 10.0, 10.0, 100.0 / glv.Re, 100.0 / glv.Re, 100.0, 100.0 * DPH,
                100.0 * DPH, 100.0 * DPH, 1.0 * glv.mg, 1.0 * glv.mg, 1.0 * glv.mg);
    Qt.Set2(1 * glv.dpsh, 1 * glv.dpsh, 1 * glv.dpsh, 100.0 * glv.ugpsHz, 100.0 * glv.ugpsHz, 100.0 * glv.ugpsHz, 0.0,
            0.0, 0.0, 0.0 * glv.dphpsh, 0.0 * glv.dphpsh, 0.0 * glv.dphpsh, 0.0 * glv.ugpsh, 0.0 * glv.ugpsh,
            0.0 * glv.ugpsh);
...
int CAligntrkang::Update(const CVect3 *pwm, const CVect3 *pvm, int nSamples, double ts, const CVect3 &vnr, int nSteps)
{
    ...
    if (vel > 1.0e-3)
    {
        ...
        if (velPre > vel0 && vel > vel0 && wz < wz0 && dyaw < dyaw0)
        { // yaw meas
            Zk.dd[3] = -diffYaw(sins.att.k, yaw);
            if (fabs(Zk.dd[3]) > 20 * DEG)
            { // yaw init
                cntYawOK = 0;
                SetYaw(yaw);
                sins.vn = vnr;
...
int CUartPP::pop(unsigned char *buf0)
{
    ...
    while (1)
    {
        if ((pushIdx > popIdx && popIdx + frameLen < pushIdx) ||
            (pushIdx < popIdx && popIdx + frameLen < pushIdx + UARTBUFLEN))
        {
...
void CConUart::dispUart(int itv)
{
    ...
    printf("\n\t�ش�(uT)\t%10.3f%10.3f%10.3f\t \n\t��ѹ(mbar)\t%10.3f\t", ps->magx, ps->magy, ps->magz, ps->bar);
    printf("\n\n\t��̬��(deg)\t%10.3f%10.3f%10.3f\t \n\t�ٶ�(m/s)\t%10.3f%10.3f%10.3f\t "
           "\n\tλ��(deg|m)\t%12.6lf%12.6lf%10.3f\t",
           ps->pch, ps->rll, ps->yaw, ps->ve, ps->vn, ps->vu, (double)ps->lon0 + ps->lon1, (double)ps->lat0 + ps->lat1,
           ps->hgt);
    printf("\n\n\tGPS�ٶ�(m/s)\t%10.3f%10.3f%10.3f\t \n\tGPSλ��(deg|m)\t%12.6lf%12.6lf%10.3f\t", ps1.gpsve, ps1.gpsvn,
           ps1.gpsvu, (double)ps1.gpslon0 + ps1.gpslon1, (double)ps1.gpslat0 + ps1.gpslat1, ps1.gpshgt);
