...
static int readfcbh(FILE *fp, nav_t *nav)
{
    ...
    while (fgets(buff, MAXRNXLEN, fp))
    {
        ...
        {
            if (!strncmp(buff, "WL", 2) && (sat = satid2no(buff + 4)) && sscanf(buff + 14, "%lf", &bias) == 1)
            {
                nav->wlbias[sat - 1] = bias;
            }
            else if (!strncmp(buff, "EL", 2) && (sat = satid2no(buff + 4)) && sscanf(buff + 14, "%lf", &bias) == 1)
            {
...
extern void readsnx(const char *snxfile, const char *infile, const char *outfile)
{
    ...
    while (fgets(buff, sizeof(buff), fp))
    {
        ...
        else if (!strncmp(buff, "+SOLUTION/ESTIMATE", 18))
        {
            while (fgets(buff, sizeof(buff), fp))
            {
                if (!strncmp(buff, "-SOLUTION/ESTIMATE", 18))
                    break;
    ...
    for (int i = 0; i < stas.n; i++)
    {
        if (stas.data[i].name[0] != '\0')
        {
            ...
            fprintf(fp_out, "%s %15.6f %15.6f %15.6f\n", stas.data[i].name, stas.data[i].pos[0], stas.data[i].pos[1],
                    stas.data[i].pos[2]);
        }
    }
...
extern int readproduct(const prcopt_t *prcopt, const filopt_t *fopt, nav_t *nav, pcvs_t *pcvss, pcvs_t *pcvsr,
                       stas_t *stas)
{
    int i, j;
    char *ext, path[1024];
...
/* read obs data ----------------------------------------------------- */
extern int readobs(const char *infile, int rcv, const prcopt_t *popt, obs_t *obs, sta_t *sta, int *nepoch)
{
    trace(3, "readobs:  n=%d\n");
...
extern void decode_corr(const char *file, nav_t *nav, int dt, int opt)
{
    ...
    while (fgets(buff, sizeof(buff), fp))
    {
        ...
        if (!init)
        {
            ...
            {
                if (sscanf(buff, "$POS%d%lf%d%d%lf%lf%lf\n", &week, &tow, &solq, &nsat, &xyz[0], &xyz[1], &xyz[2]) < 7)
                    continue;
                ecef2pos(xyz, blh);
