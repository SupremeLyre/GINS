                    ...
                    sta_t *sta, char *phaseshift)
{
    ...
    *sys = SYS_GPS;
    ...
    while (fgets(buff, MAXRNXLEN, fp))
    {
        ...
        {
        case 'O':
            decode_obsh(fp, buff, *ver, tsys, tobs, nav, sta, phaseshift);
            break;
        case 'N':
...
static int decode_obsdata(FILE *fp, char *buff, double ver, int mask, sigind_t *index, obsd_t *obs)
{
    ...
    for (i = 0; i < ind->n; i++)
    {
        if (p[i] < 0 || val[i] == 0.0 || p[i] > NFREQ + NEXOBS - 1)
            continue;
        switch (ind->type[i])
        {
        case 0:
...
/* set signal index ----------------------------------------------------------*/
static void set_index(double ver, int sys, const char *opt, char tobs[MAXOBSTYPE][4], sigind_t *ind, char *phaseshift)
{
    const char *p;
    char str[8], *optstr = "";
    double shift;
...
static int readrnxobsb(FILE *fp, const char *opt, double ver, int *tsys, char tobs[][MAXOBSTYPE][4], int *flag,
                       obsd_t *data, sta_t *sta, char *phaseshift)
{
    gtime_t time = {0};
    sigind_t index[7] = {{0}};
    char buff[MAXRNXLEN];
                      ...
                      char tobs[][MAXOBSTYPE][4], obs_t *obs, sta_t *sta, char *phaseshift)
{
    ...
    while ((n = readrnxobsb(fp, opt, ver, tsys, tobs, &flag, data, sta, phaseshift)) >= 0 && stat >= 0)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            /* utc -> gpst */
            if (*tsys == TSYS_UTC)
                data[i].time = utc2gpst(data[i].time); // ϵͳUTCʱ
                     ...
                     obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    /* read rinex header */
    if (!readrnxh(fp, &ver, type, &sys, &tsys, tobs, nav, sta, phaseshift))
        return 0;
    ...
    {
    case 'O':
        return readrnxobs(fp, ts, te, tint, opt, index, ver, &tsys, tobs, obs, sta, phaseshift);
    case 'N':
...
extern int input_rnxctr(rnxctr_t *rnx, FILE *fp)
{
    ...
    if (rnx->type == 'O')
    {
        if ((n = readrnxobsb(fp, rnx->opt, rnx->ver, &rnx->tsys, rnx->tobs, &flag, rnx->obs.data, &rnx->sta, NULL)) <=
            0)
        {
            rnx->obs.n = 0;
            return n < 0 ? -2 : 0;
