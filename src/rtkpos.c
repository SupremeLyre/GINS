...
extern void outrel(FILE *fp, const rtk_t *rtk)
{
    ...
    if (!fp || rtk->sol.stat == SOLQ_NONE)
        return;

    /* write solution status */
    n = reloutstat(rtk, buff);
    buff[n] = '\0';
    ...
    for (i = 0; i < MAXSAT; i++)
    {
        ...
        for (j = 0; j < nfreq; j++)
        {
            fprintf(fp, "$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%d,%d,%d\n", week, tow, id, j + 1,
                    ssat->azel[0] * R2D, ssat->azel[1] * R2D, ssat->resp[j], ssat->resc[j], ssat->vsat[j], ssat->fix[j],
                    ssat->slip[j] & 3, ssat->lock[j], ssat->outc[j]);
        }
    }
}
...
static void sdobs(const nav_t *nav, const obsd_t *obs, int iu, int ir, double *L, double *P, double *Lc, double *Pc)
{
    int i, j, k;
    ...
    for (k = 0; k < NFREQ; k++)
    {
        ...
        if (Pi != 0.0 && Pj != 0.0)
            P[k] = Pi - Pj;
        if (Li != 0.0 && Lj != 0.0 && freq[k] != 0.0)
            L[k] = CLIGHT * (Li - Lj) / freq[k];
    }
...
/* LC observation --------------------------------------------------------------- */
static void lc_rel(rtk_t *rtk, const obsd_t *obs, const int *sat, const int *iu, const int *ir, int ns,
                   const nav_t *nav)
{
    int i, k;
    ...
    for (i = 0; i < ns; i++)
    {
        ...
        for (k = 0; k < NFREQ; k++)
            freq[k] = sat2freq(sat[i], obs[iu[i]].code[k], nav);
        ...
        if (freq[1] * freq[2] * L[1] * L[2] * P[0] * P[1] != 0.0)
        {
            ...
            lambda = CLIGHT / (freq[1] - freq[2]);
            rtk->ssat[sat[i] - 1].LC[4] =
                (freq[1] * L[1] - freq[2] * L[2]) / CLIGHT - ((1 - alfa) * P[0] + alfa * P[1]) / lambda;
        }
    }
}
...
static uint8_t detslp_ll(rtk_t *rtk, const obsd_t *obs, int i, int rcv)
{
    ...
    for (f = 0; f < NF(&rtk->opt); f++)
    {
        ...
        if (obs[i].L[f] == 0.0 || fabs(timediff(obs[i].time, rtk->ssat[sat - 1].prev_T[rcv - 1])) < DTTOL)
        {
            continue;
        }
...
/* cycle Jump detect ------------------------------------------------------- */
static void cycJump_detect(rtk_t *rtk, const obsd_t *obs, const int *sat, const int *iu, const int *ir, int ns,
                           const nav_t *nav)
{
...
/* update information -------------------------------------------------------- */
static void update_sat_amb(rtk_t *rtk, int *stat, const obsd_t *obs, const int *sat, const int *iu, const int *ir,
                           int ns)
{
                   ...
                   const nav_t *nav)
{
    ...
    for (f = 0; f < NF(&rtk->opt); f++)
    {
        ...
        for (i = 0; i < ns; i++)
        {
            ...
            rtk->x[j] = 0.0;
            if (bias != 0.0)
                initx(rtk, bias, SQR(rtk->opt.P0[5]), j);
        }
    }
}
...
static void zdres(int base, const obsd_t *obs, int n, const double *rs, const double *dts, const double *var,
...
static int ddres(rtk_t *rtk, const nav_t *nav, double dt, const double *x, const int *sat, double *y, double *e,
                 double *azel, double *freq, const int *iu, const int *ir, int ns, double *v, double *H, double *R,
                 int *vflg, exc_t *exc)
{
    prcopt_t *opt = &rtk->opt;
    ...
    { /* m=0:GPS,1:GLO,2:GAL,3:BDS */
        ...
        for (f = 0; f < nf * 2; f++)
        {
            ...
            for (j = 0; j < ns; j++)
            {
                ...
                if (!validobs(iu[j], ir[j], f, nf, y))
                    continue;
                if (exclude(sat[i], sat[j], f, nf, exc))
                    continue;
        ...
        for (f = 0, k = na; f < nf; f++, k += MAXSAT)
        {
            ...
            for (j = k; j < k + MAXSAT; j++)
            {
                ...
                if (rtk->ssat[j - k].lock[f] > rtk->opt.minlock && !(rtk->ssat[j - k].slip[f] & 2) &&
                    rtk->ssat[i - k].vsat[f] && rtk->ssat[j - k].azel[1] >= rtk->opt.elmaskar && !nofix)
                {
    ...
    if (stat == 0)
    {
        ...
        for (i = 0; i < na; i++)
        {
            ...
            if (type & 3)
            {
                ...
                {
                case 1:
                    rtk->ssat[sat2 - 1].resc[freq - 1] = v[i];
                    rtk->ssat[sat1 - 1].vsat[freq - 1] = 1;
                    rtk->ssat[sat2 - 1].vsat[freq - 1] = 1;
                    break;
                case 2:
                    rtk->ssat[sat2 - 1].resp[freq - 1] = v[i];
                    break;
                }
            }
        }
...
extern void relpos(rtk_t *rtk, const obsd_t *obs, int nu, int nr, const nav_t *nav)
{
    ...
    for (i = 0; i < MAXSAT; i++)
    {
        ...
        for (j = 0; j < nf; j++)
        {
            rtk->ssat[i].vsat[j] = rtk->ssat[i].fix[j] = 0;
    ...
    zdres(1, obs + nu, nr, rs + nu * 6, dts + nu * 2, var + nu, svh + nu, nav, rtk->rb, opt, 1, y + nu * nf * 2,
