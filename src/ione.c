    ...
    while (fgets(buff, sizeof(buff), fp))
    {
        ...
        else if (strstr(label, "LAT/LON1/LON2/DLON/H") == label && p)
        {
            ...
            for (m = 0; m < n; m++)
            {
                ...
                if (type == 1)
                    p->data[index] = x * pow(10.0, nexp); // pow(10.nexp):����λת��ΪTECU
                else
...
extern int pppcorr_stec(gtime_t time, const double *blh, const int sat1, const int sat2, double *stec, double *var,
