...
CGNSSManage::CGNSSManage()
{
    ...
    {
        if (sscanf(buff, "%lf%d%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf\n", &ti, &q, &pos[0], &pos[1], &pos[2],
                   &vel[0], &vel[1], &vel[2], &att[0], &att[1], &att[2], &eb[0], &eb[1], &eb[2], &db[0], &db[1],
                   &db[2]) < 17)
            continue;
        ...
        data.rr[1] = pos[1];
        data.rr[2] = pos[2];
        data.qr[0] = 0.05 / RE_WGS84;
        data.qr[1] = 0.05 / RE_WGS84;
        data.qr[2] = 0.1;
...
int CGNSSManage::Init(const prcopt_t &popt, const solopt_t &sopt, const filopt_t &fopt, const sta_t &sta)
{
    prcopt_t popt_ = popt;
    ...
    /* read product */
    if (!readproduct(&popt_, &fopt, &nav, &pcvss, &pcvsr, NULL))
        return 0;
...
void CGNSSManage::Update(obsd_t *obs, int nobs, int bimu2gps, const double *ins_xyz, const double *ins_Pxyz)
{
    ...
    {
        sat = obs[i].sat;
        if (!(rtk.ssat[sat - 1].sys & prcopt_->navsys))
            continue;
    ...
    if (time0.time != 0)
        rtk.tt = fabs(timediff(rtk.sol.time, time0));
    ...
    ecef2pos(rtk.sol.rr, blh);
    matcpy(pos_blh, blh, 1, 3);
    ecef2enu(blh, rtk.sol.rr + 3, venu);
    matcpy(vel_enu, venu, 1, 3);
...
void CGNSSManage::OutGPSResult()
{
    ...
    {
        float std[] = {0.05, 0.1};
        rk_blh[0] = rk_blh[1] = std[0] / RE_WGS84;
        rk_blh[2] = std[1];
    }
...
{
    return ((ts.time == 0 || timediff(time, ts) >= -DTTOL) && (te.time == 0 || timediff(time, te) < DTTOL));
}
...
void removeUnusedData(gtime_t &ts, gtime_t &te, nav_t *nav, pcvs_t *pcvss, pcvs_t *pcvsr)
{
    ...
    for (int n = 0; n < nav->ntrop; n++)
    {
        ...
        if (!(nav_trop = (trop_t *)realloc(ptrop->data, sizeof(trop_t) * ptrop->nt)))
        {
            free(ptrop->data);
            ptrop->data = NULL;
            ptrop->nt = ptrop->ntmax = 0;
            trace(1, "combtrop malloc error nt=%d\n", ptrop->nt);
        }
        else
        {
            ptrop->data = nav_trop;
            ptrop->ntmax = ptrop->nt;
        }
    }
    ...
    for (int n = 0; n < nav->nstec; n++)
    {
        ...
        if (!(nav_stec = (stec_t *)realloc(pstec->data, sizeof(stec_t) * pstec->ns)))
        {
            free(pstec->data);
            pstec->data = NULL;
            pstec->ns = pstec->nsmax = 0;
            trace(1, "combstec malloc error ns=%d\n", pstec->ns);
        }
        else
        {
            pstec->data = nav_stec;
            pstec->nsmax = pstec->ns;
        }
    }
}