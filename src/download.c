...
static int test_file(const char *local)
{
    ...
    if ((p = strrchr(buff, '.')) && (!strcmp(p, ".z") || !strcmp(p, ".gz") || !strcmp(p, ".zip") || !strcmp(p, ".Z") ||
...
static int add_path(paths_t *paths, const char *remot, const char *dir)
{
    ...
    if (!(paths->path[paths->n].remot = (char *)malloc(1023 /*strlen(remot)*/ + 1)) ||
        !(paths->path[paths->n].local = (char *)malloc(1023 /*strlen(local)*/ + 1)))
    { //�����ϵ����
        free_path(paths);
        return 0;
    }
...
static int gen_path(gtime_t time, gtime_t time_p, int seqnos, int seqnoe, const url_t *url, const char *sta,
                    const char *dir, paths_t *paths)
{
...
static int gen_paths(gtime_t time, gtime_t time_p, int seqnos, int seqnoe, const url_t *url, char **stas, int nsta,
                     const char *dir, paths_t *paths)
{
    int i;
...
/* execute download ----------------------------------------------------------*/
static int exec_down(const path_t *path, char *remot_p, const char *usr, const char *pwd, const char *proxy, int opts,
                     int *n, FILE *fp)
{
    ...
    /* get remote file list */
    if ((p = strrchr(path->remot, '/')) && strncmp(path->remot, remot_p, p - path->remot))
    {
    ...
    else
    {
        ...
        sprintf(cmd, "%s%s %s %s-t %d -T %d -O \"%s\" -o \"%s\"%s\n", env, FTP_CMD, path->remot, opt, FTP_RETRY,
                FTP_TIMEOUT, path->local, errfile, opt2);
    }
    if (fp)
    ...
    if (!(opts & DLOPT_KEEPCMP) && (p = strrchr(path->local, '.')) &&
        (!strcmp(p, ".z") || !strcmp(p, ".gz") || !strcmp(p, ".zip") || !strcmp(p, ".Z") || !strcmp(p, ".GZ") ||
         !strcmp(p, ".ZIP")))
    {
...
/* test local file -----------------------------------------------------------*/
static int test_local(gtime_t ts, gtime_t te, double ti, const char *path, const char *sta, const char *dir, int *nc,
                      int *nt, FILE *fp)
{
    gtime_t time;
...
/* test local files ----------------------------------------------------------*/
static int test_locals(gtime_t ts, gtime_t te, double ti, const url_t *url, char **stas, int nsta, const char *dir,
                       int *nc, int *nt, FILE *fp)
{
    int i;
 ...
 *-----------------------------------------------------------------------------*/
extern int dl_exec(gtime_t ts, gtime_t te, double ti, int seqnos, int seqnoe, const url_t *urls, int nurl, char **stas,
 ...
 * return : status (1:ok,0:error,-1:aborted)
 *-----------------------------------------------------------------------------*/
extern void dl_test(gtime_t ts, gtime_t te, double ti, const url_t *urls, int nurl, char **stas, int nsta,
