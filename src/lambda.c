...
/* lambda par */
extern int lambda_PAR(rtk_t *rtk, int na, int nb, int m, const double *a, double *b, const double *Qaa, double *Qbb,
                      const double *Qba)
{
    ...
    if (!(info = LD(na, Qaa, L, D)))
    {
        ...
        for (k = na; k >= 4; k--)
        {
            ...
            /* mlambda search */
            if (!(info = search(k, m, Lp, Dp, zp, F, s)))
            { /* returns 0 if no error */
