...
static int rcv2sta[MAXRCV] = {0}, rcv2ind[MAXRCV][4] = {0}, ind2rcv[MAXRCV * 4] = {0}, ind2sys[MAXRCV * 4] = {0};
static int nexp = -1;
...
static void geo2solarmag(const double *ipp_geo, double tt, double *solar_mag)
{
    ...
    sun_geo[0] = 0;
    sun_geo[1] = PI - tt * PI / 43200;
    geo2mag(sun_geo, sun_mag);
    solar_mag[0] = ipp_mag[0];
    solar_mag[1] = ipp_mag[1] - sun_mag[1];

    if (solar_mag[1] < -PI)
        solar_mag[1] += 2 * PI;
    else if (solar_mag[1] > PI)
        solar_mag[1] -= 2 * PI;
}
...
static void P_nm(double x, double *P, int N)
{
    ...
    double a = 0, b = 0, c = 0;
    P[0 + 0] = 1;
    P[0 + 1 * N] = sqrt(3) * cos(x);
    P[1 + 1 * N] = -sqrt(3) * sin(x); // P[1 + 1 * N] = sqrt(3)*sin(x);
    for (n = 2; n < N; n++)
    {
...
static int file2tec(const char *infile, const sta_t *sta, int ircv, unsigned char *sflag, unsigned char *syslist)
{
    ...
    *syslist = 0;
    ...
    {
        if (sscanf(buff, "%s %s %s %lf %lf %lf %lf %lf", &ymd, &dms, satid, &azel[0], &azel[1], &stec1, &stec2, &std) <
            8)
        {
            continue;
        }
        ...
        if (timediff(ts, ti) > 0.0f || timediff(ti, te) > 0.0f || azel[1] < 15 * D2R || !(sys & optsys))
            continue;
...
static int generateData(const char *infile)
{
    ...
    for (int i = 0, ircv = 0; i < stas.n; i++)
    {
        ...
        if (!file2tec(stafile, &stas.data[i], ircv, sflag, &syslist))
            continue;
...
static int estiono()
{
    ...
    for (int i = 0; i < tec.n; i++)
    {
        ...
        checkbrk("(%3d)%s %s %4d %6.3f", ind2rcv[ir], stas.data[rcv2sta[ind2rcv[ir]]].name,
                 time_str(tec.data[i].time, 0), iperiod, dt);
    }
    checkbrk("LS...\n");
...
static int postiono(const char *outdir)
{
    ...
    for (int i = 0; i < tec.n; i++)
    {
        ...
        fprintf(fp, "%s %6s %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f\n", time_str(tec.data[i].time, 0), id, vtec,
                vtec_cal, fabs(vtec - vtec_cal), l_tec, stec, fabs(l_tec - stec));
    }
...
extern int ionpos(const char *infile, const char *outdir)
{
    ...
    /* output */
    if (!outionogrid(outdir, lat, lon, 1))
        return 0;
