...
{
    alni0.Init(sins0.pos);
    CSINSTDKF::Init(sins0);

    posInit = 1;
    velPre = yawPre = cntYaw = alnkfInit = 0;
    sins.mvnT = 0.1;
    ...
    if (!IsZero(posgnss) && avpi.Interp(posGNSSdelay + dtGNSSdelay, 0x4))
    {
        *(CVect3 *)&Zk.dd[3] = avpi.pos - posgnss;
        trace(2, "%10.2f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f\n",
              sins.tk, Zk.dd[3] * glv.Re, Zk.dd[4] * glv.Re, Zk.dd[5], sqrt(Rt.dd[3]) * glv.Re, sqrt(Rt.dd[4]) * glv.Re,
              sqrt(Rt.dd[5]), sqrt(Pk(6, 6)) * glv.Re, sqrt(Pk(7, 7)) * glv.Re, sqrt(Pk(8, 8)), sqrt(Pk(3, 3)),
              sqrt(Pk(4, 4)), sqrt(Pk(5, 5)));
        SetMeasFlag(00070);
    }
}
...
int CMyAutoDrive::Init(const prcopt_t &popt, const solopt_t &sopt, const filopt_t &fopt, const sta_t &sta,
                       const imuopt_t &iopt)
{
    /* Basic Initialize */
...
void CMyAutoDrive::GPSProcess(obsd_t *obs, int n)
{
    ...
    if (mState == NAVIGATION)
    {
        ...
        X[1] = mpFIR->pos_xyz.j;
        X[2] = mpFIR->pos_xyz.k;
        P[0] = mpFIR->Rk_xyz.e00;
        P[1] = mpFIR->Rk_xyz.e01;
        P[2] = mpFIR->Rk_xyz.e02;
        P[3] = mpFIR->Rk_xyz.e10;
        P[4] = mpFIR->Rk_xyz.e11;
        P[5] = mpFIR->Rk_xyz.e12;
        P[6] = mpFIR->Rk_xyz.e20;
        P[7] = mpFIR->Rk_xyz.e21;
        P[8] = mpFIR->Rk_xyz.e22;
