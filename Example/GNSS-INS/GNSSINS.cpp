...
int main(int argc, char *argv[])
{
    ...
    if (!Gyaml.LoadYAML(argv[1]) || !Iyaml.LoadYAML(argv[1]))
        return -1;

    /* Read Data */
    if (!readobs(Gyaml.filopt.rovobs, 1, &Gyaml.prcopt, &obss, &sta, &nepoch))
        return 0;
    if (!readimu(&Iyaml.imuopt, &imus))
        return 0;
    ...
    if (!data_align(memimu, memobs, iimu, iobs))
        return 0;

    /* Processing instantiation */
    CMyAutoDrive app;
    if (!(app.Init(Gyaml.prcopt, Gyaml.solopt, Gyaml.filopt, sta, Iyaml.imuopt)))
        return 0;
