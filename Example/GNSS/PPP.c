...
static int process(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    if (time.time != 0)
        rtk->tt = fabs(timediff(rtk->sol.time, time));
...
/* process positioning ----------------------------------------------------*/
static void procpos(const prcopt_t *popt, const solopt_t *sopt, rtk_t *rtk, int mode, FILE *fp_outs[])
{
    int i, nobs, n;
    obsd_t obs[MAXOBS];
    ...
    while ((nobs = inputobs(obs, rtk->sol.stat, popt)) >= 0)
    {
        ...
        for (i = n = 0; i < nobs; i++)
        {
            if (!(rtk->ssat[obs[i].sat - 1].sys & popt->navsys))
                continue;
            if (popt->exsats[obs[i].sat - 1])
                continue;
...
int main(int argc, char *argv[])
{
    ...
    if (!(fp_stas = fopen("fcb-enu-0522.txt", "w")))
        return 0;
    ...
    /* read product */
    if (!readproduct(&prcopt, &filopt, &navs, &pcvss, &pcvsr, &stas))
        return 0;
    ...
    for (int i = 0; i < stas.n; i++)
    {
        ...
        if (!readobs(path, 1, &prcopt, &obss, &sta, &nepoch))
            continue;
        ...
        if (prcopt.soltype == 0)
        {
            ...
            for (int ii = 0; ii < 3; ii++)
                rr[ii] = rtk.sol.rr[ii] - prcopt.ru[ii];
            ecef2enu(pos, rr, enu);
            ...
            fflush(fp_stas);
#else
            fprintf(fp_stas, "%s %15.4f %15.4f %15.4f\n", stas.data[i].name, rtk.sol.rr[0], rtk.sol.rr[1],
                    rtk.sol.rr[2]);
            fflush(fp_stas);
#endif
            rtkfree(&rtk);
